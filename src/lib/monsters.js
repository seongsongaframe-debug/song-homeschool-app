...
export function makeEgg(studentId, id, now = () => new Date().toISOString()) {
    return {
        id,
...
export function applyProgress(inst, delta, now = () => new Date().toISOString(), rng = Math.random) {
    if (delta <= 0)
        return inst;
    let next = { ...inst, progress: inst.progress + delta };
    if (!next.species_id) {
        if (next.progress < EGG_HATCH_AT)
            return next;
...
function advance(inst, sp) {
    let stage = inst.stage;
    for (let i = stage; i < sp.stages.length; i++) {
        if (inst.progress >= sp.stages[i].questsToReach)
            stage = i + 1;
        else
            break;
    }
