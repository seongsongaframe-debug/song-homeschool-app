...
interface LegacyQuest extends Omit<Quest, "due_date" | "assigned_date"> {
  date?: string;
  due_date?: string;
  assigned_date?: string;
}

export async function migrateQuestsIfNeeded(): Promise<void> {
  const flag = await storage.read<{ done: boolean; at: string }>(
    KEYS.questMigrationFlag
  );
  if (flag?.done) return;
  ...
  for (const key of allKeys) {
    ...
    if (!DATE_RE.test(maybeDate)) continue;

    const legacy = await storage.read<LegacyQuest>(key);
    if (!legacy) {
    ...
    delete (migrated as Partial<LegacyQuest>).date;
