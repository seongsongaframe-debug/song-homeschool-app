...
import type { Difficulty, Quest, Subtask } from "../types";
...
const AMOUNT_PATTERNS: { re: RegExp; unit: string }[] = [
  { re: /(\d+)\s*회/, unit: "회" },
  { re: /(\d+)\s*번/, unit: "번" },
  { re: /(\d+)\s*쪽/, unit: "쪽" },
  { re: /(\d+)\s*페이지/, unit: "페이지" },
...
function isLikelySection(s: string): boolean {
  if (s.length > SECTION_HEADER_MAX_LEN) return false;
  if (/\d/.test(s)) return false;
  return true;
}

function buildQuest(body: string, section?: string): ParsedQuest {
...
function extractTargetUnit(q: ParsedQuest) {
