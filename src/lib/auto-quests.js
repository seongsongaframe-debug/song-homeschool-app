...
async function ensureWeekQuests(studentId, weekStartISO, now) {
    ...
    let created = 0;
    for (const [wd, specs] of Object.entries(HYEIN_WEEKLY)) {
...
// 토/일 첫 로드 시 차주 자동 부여.
export async function maybeAutoSeedHyein(now = new Date()) {
...
export async function manualSeedHyein(now = new Date()) {
    const day = now.getDay();
